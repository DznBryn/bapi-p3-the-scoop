...
function createComment(url, request) {
  ...
  if(requestComment
    && (Object.keys(requestComment).length >= 1)
...
function updateComment(url, request){
  ...
  const hasBody = updateComment && updateComment.body.length;
  const response = {};

  if(!id || !updateComment || !hasBody){
    response.status = 400;
  } else if (!dbComment) {
    response.status = 404;
  } else {
    dbComment.body = updateComment.body || dbComment.body;
...
function deleteComment(url, request) {
    ...
    if (dbComment) {
      ...
      const commentToDelete = userCommentIds.indexOf(id);
      userCommentIds.splice(commentToDelete, 1);
